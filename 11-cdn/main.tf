resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}"#web-dev.kithusdairy.fun
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]   #web-cdn.kithusdairy.fun
  ...
  viewer_certificate {
    acm_certificate_arn = data.aws_ssm_parameter.web_acm_arn.value
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.kithusdairy.fun
      type    = "A"
